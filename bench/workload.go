...
// below is the point-access slice of the YCSB core (the lingua franca that makes kv numbers
// comparable to the published RocksDB/LMDB/LevelDB literature) plus the two targeted micro
// workloads the spec calls out: a cold bulk load and a write-saturated ingest (spec 21 §2).
// The range-scan workload (YCSB-E) is not part of this set, since kv exposes only point
// operations.
//
...
// the point-access YCSB workloads (A, B, C, D, F) plus the two targeted micro workloads, a
// cold bulk load and a write-saturated ingest.
