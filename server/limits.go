...
// hurt every other client and the process itself: a gigabyte value or a batch of millions of ops
// turns one request into a memory or latency spike the single writer cannot absorb. Limits are the
// guardrails that keep one process serving many clients safely.
//
...
// refused: a 64 KiB key, a 16 MiB value, and a hundred thousand ops per batch. A deployment with
// different needs sets its own through Options.Limits.
