...
- **Durability is one field.** `Options.SyncWrites` picks the contract, and both settings are durable. The default is background group commit, a bounded sub-second loss window, the same contract Redis gives with `appendfsync everysec`. Setting it true is synchronous group commit, where a write waits for the group-commit fsync before it returns, for zero acked-commit loss, the same contract as `appendfsync always`. This replaces the old `SyncOff` to `SyncExtra` ladder. See the [durability guide](/guides/durability/).
- **The server is Redis-only, with redis-server flags.** The `kv` binary serves one store over the Redis wire protocol. Its flags follow `redis-server`, so `--port`, `--bind`, `--unixsocket`, `--dir`, `--dbfilename`, `--appendonly`, `--appendfsync`, and `--maxmemory` carry their redis meaning and the binary is close to a drop-in. `--appendfsync always` opens the store in the zero-loss mode; `everysec` is the default. The HTTP and binary servers, the auth, the TLS, and the change feed are gone. See the [server guide](/guides/server/).
