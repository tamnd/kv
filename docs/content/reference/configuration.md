...
| `SyncWrites` | `false` | `true` or `false` | Durability contract. `false` is background group commit, a bounded sub-second loss window; `true` fsyncs every commit before it returns for zero acked-commit loss. See [durability](/guides/durability/). |
...
| File | Role |
| --- | --- |
| `app.kv` (or `<dir>/kv.db` under the server) | The main store file: the hash-indexed log where keys and values live. |
