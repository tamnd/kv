...
// ringLog is the lock-free append-only core. tail is the only write-path state: it is the
...
// newRingLog returns a log backed by capBytes of memory. capBytes bounds the total record
// bytes this step can hold, since there is no spill yet; the engine sizes it to the
// working set under test.
func newRingLog(capBytes int64) *ringLog {
	return &ringLog{buf: make([]byte, capBytes), cap: capBytes}
}
...
func (l *ringLog) Reserve(n int) (int64, []byte) {
...
func (l *ringLog) Append(rec []byte) int64 {
...
func (l *ringLog) At(addr int64) []byte {
...
func (l *ringLog) Tail() int64 { return l.tail.Load() }
