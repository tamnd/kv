...
// coldDB is the larger-than-memory point store: the hybrid log holds the records with its hot
...
// openColdDB creates a store at path whose log keeps ringBytes of the recent tail resident and
...
func openColdDB(path string, ringBytes int64, capKeys int) (*coldDB, error) {
...
func (d *coldDB) Set(key, value []byte) { d.write(opSet, key, value) }
...
func (d *coldDB) Delete(key []byte) { d.write(opDel, key, nil) }
...
func (d *coldDB) write(op byte, key, value []byte) {
...
func (d *coldDB) Get(key, scratch []byte) ([]byte, bool, error) {
...
func (d *coldDB) get(key, scratch []byte) (val []byte, ok, fromDisk bool, err error) {
...
func (d *coldDB) Sync() error { return d.log.Sync() }

// Close flushes and closes the backing file. After Close every set is on disk.
func (d *coldDB) Close() error { return d.log.Close() }
