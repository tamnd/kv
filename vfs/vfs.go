// Package vfs is the file-I/O seam for kv (spec 03 §5). Every read and write the
...
type File interface {
	...
	// failed Sync is fatal to durability (fsyncgate, spec 07): callers must not
...
// SQLite uses 32 KiB regions; kv matches that so the -shm layout is familiar.
